...
Before a paid cloud submission, prefer a free local dry-run when the shot is previewable locally: Wan 2.2 T2V 14B (text to video, 832×480/3 s ≈ 2.10 min), MiniMax H3 (T2V/I2V with native AAC 32 kHz audio, ≈ 2.0–2.3 min), Wan 2.2 I2V 14B (≈ 1.50 min), and Wan2ReferenceVideoApi / MiniMaxH3ReferenceToVideo for reference consistency. The local preview exists to catch prompt, identity, structure, and pacing problems before spending cloud quota; it does not replace the context-submission package, the explicit model-selection gate, or per-shot authorization. Submit to the cloud only when the local chain cannot meet the requested model/quality/duration or the user explicitly chooses cloud.
