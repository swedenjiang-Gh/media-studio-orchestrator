...
For whole-person replacement, the local route is reconstruction rather than a dedicated face-only swap: authorized target references → PuLID identity conditioning → Union ControlNet/depth or pose structure → per-shot Wan I2V. The 2026-08-13 five-segment Wan trial improved a 15.57-second action over one repetitive I2V pass, but has no verified tail-frame binding; segment joins, identity, clothing/background continuity, and flicker remain per-shot acceptance work. ReActor, SimSwap, and FaceFusion are not installed usable chains, so a request specifically requiring one of them is `missing` rather than silently rerouted.
