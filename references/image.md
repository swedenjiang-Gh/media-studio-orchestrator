...
For project work, save the final image under the project's existing asset contract. For a standalone native ComfyUI image job, use `D:\MediaStudio\Comfyui\Image\<job>`. For a standalone GPT Image job, including GPT Image semantic edits, download the generated image and record to `D:\MediaStudio\GPT-Imag\<job>`; a remote URL alone is not a completed local delivery. For standalone ImageMagick, rembg, Sharp, or Jianying-last-frame work, use `D:\MediaStudio\Image\<job>`. Jianying project output stays in the project contract.
